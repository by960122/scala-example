...
object Solution9 {
  ...
  def isPalindrome(content: Int): Boolean = {
    if (content < 0 || content % 10 == 0 && content != 0) {
      return false
    }
    var currentInt = content;
    var reverseInt = 0
    while (currentInt > reverseInt) {
      reverseInt = reverseInt * 10 + currentInt % 10;
      currentInt /= 10
    }
    currentInt == reverseInt || currentInt == reverseInt / 10
  }
}
